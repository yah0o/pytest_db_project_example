...
def test_fetch_categories_no_title(capi, yaml_config, title_code, update_titles):
  ...
  assert_that(response.code, equal_to(ContractError.COMMERCE_TITLE_NOT_FOUND_ERROR),
              allure_name='response has expected error code')
  assert_that(response.context['reason'], equal_to('Title is not found.'),
              allure_name='response has expected error reason')

  assert_that(response.context['context']['title_code'], equal_to(title_code),
              allure_name='Expected correct title_code in context')

@allure.feature('capi')
@allure.story('fetch_categories_not_catalog')
def test_fetch_categories_no_catalog(capi, yaml_config):
    response = capi.commerce.fetch_storefront_categories_v2(title_code=yaml_config.data.NO_CATALOG_TITLE_CODE,
                                                            storefront=yaml_config.data.STOREFRONT_WITH_TWO_CATEGORIES.CODE)
    assert_that(type(response), equal_to(CAPIResponseError), allure_name='response has expected type')
    assert_that(response.code, equal_to(ContractError.COMMERCE_CATALOG_NOT_FOUND_ERROR),
                allure_name='response has expected error code')

    assert_that(response.context['reason'], equal_to('Catalog is not found.'),
                allure_name='response has expected error reason')

    assert_that(response.context['context']['title_code'], equal_to(yaml_config.data.NO_CATALOG_TITLE_CODE),
                allure_name='Expected correct title_code in context')


@allure.feature('capi')
@allure.story('fetch_categories_not_storefront')
def test_fetch_categories_when_storefront_not_found(capi, title_code):
    response = capi.commerce.fetch_storefront_categories_v2(title_code=title_code,
                                                            storefront='test_store_not_found')

    assert_that(type(response), equal_to(CAPIResponseError), allure_name='response has expected type')

    assert_that(response.code, equal_to(ContractError.COMMERCE_STOREFRONT_NOT_FOUND_ERROR), allure_name='response has expected code')

    assert_that(response.context['reason'], equal_to('Entity is not found.'),
                allure_name='response has expected error reason')

    assert_that(response.context['context']['entity_code'], equal_to('test_store_not_found'),
                allure_name='Expected correct entity_code in context')
    assert_that(response.context['context']['entity_type'], equal_to('STOREFRONT'),
                allure_name='Expected correct entity_type in context')
